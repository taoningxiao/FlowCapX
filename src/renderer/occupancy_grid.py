...
class OccupancyGrid():
    ...
    def save(self, path):
        ...
        data = {
            ...
            'density_grid': self.density_grid.cpu(),
            'density_bitfield': self.density_bitfield.cpu(),
            'mean_density': self.mean_density,
            'iter_density': self.iter_density,
            'grid_coords': self.grid_coords.cpu(),
            'grid_indices': self.grid_indices.cpu(),
            'morton3D_indices': self.morton3D_indices.cpu(),
            'aabb_min': self.aabb_min.cpu(),
            'aabb_max': self.aabb_max.cpu(),
            'aabb': self.aabb.cpu(),
        }
    ...
    def load(cls, path, device='cpu'):
        """Load the class attributes from a file and move tensors to the specified device."""
        data = torch.load(path, map_location=device)
        obj = cls(
            grid_size=data['grid_size'],
            density_thresh=data['density_thresh'],
            bound=data['bound'],
            aabb_min=data['aabb_min'].to(device),
            aabb_max=data['aabb_max'].to(device)
        )
        obj.cascade=data['cascade']
        obj.density_grid = data['density_grid'].to(device)
        obj.density_bitfield = data['density_bitfield'].to(device)
        obj.mean_density = data['mean_density']
        obj.iter_density = data['iter_density']
        obj.grid_coords = data['grid_coords'].to(device)
        obj.grid_indices = data['grid_indices'].to(device)
        obj.morton3D_indices = data['morton3D_indices'].to(device)
        obj.aabb = data['aabb'].to(device)
        return obj
...
class OccupancyGridDynamic():
    ...
    def save(self, path):
        ...
        data = {
            ...
            'density_grid': self.density_grid.cpu(),
            'density_bitfield': self.density_bitfield.cpu(),
            'mean_density': self.mean_density.cpu(),
            'iter_density': self.iter_density,
    ...
    def load(cls, path, device='cpu'):
        ...
        obj.density_grid = data['density_grid'].to(device)
        obj.density_bitfield = data['density_bitfield'].to(device)
        obj.mean_density = data['mean_density'].to(device)
        obj.iter_density = data['iter_density']
        obj.times = data['times'].to(device)
        obj.grid_coords = data['grid_coords'].to(device)
        obj.grid_indices = data['grid_indices'].to(device)
        obj.morton3D_indices = data['morton3D_indices'].to(device)
        obj.aabb = data['aabb'].to(device)
    ...
    def update_grid(self, query_fn, decay=0.95, S=128):
        ...
        for t in range(self.time_size):
            valid_mask = (self.density_grid[t] >= 0) & (tmp_grid[t] >= 0)
            self.density_grid[t][valid_mask] = torch.max(self.density_grid[t][valid_mask] * decay, tmp_grid[t][valid_mask])
        # torch.max(self.density_grid[valid_mask], tmp_grid[valid_mask], out=self.density_grid[valid_mask])
