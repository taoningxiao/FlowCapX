...
def render(H, W, K, model, N_samples = 64, chunk=1024*32, rays=None, c2w=None, netchunk = 1024*64,
                  ...
                  cuda_ray = False,
                  perturb = 0.,
                  vel_model = None, my_voxel_tool = None, use_grid = False, den_grid = None
                  ):
    """Render rays
    Args:
    ...
    for i in range(0, rays.shape[0], chunk):
        if cuda_ray:
            if model.single_scene:
                ret = render_rays_cuda_single_scene(rays[i:i+chunk], model, chunk = netchunk, perturb=perturb, my_voxel_tool = my_voxel_tool, use_grid=use_grid, den_grid=den_grid)
            else:
                ret = render_rays_cuda(rays[i:i+chunk], model, chunk = netchunk, perturb=perturb, vel_model = vel_model, my_voxel_tool = my_voxel_tool)
        else:
            if model.single_scene:
...
def render_rays_cuda(ray_batch,
                ...
                chunk = 1024*64,
                perturb=0.,
                vel_model = None, my_voxel_tool = None
                ):
    ...
    def get_raw(fn, pts_dynamic, pts_static):
        ...
        # for b_idx in range(0, pts_dynamic.shape[0], chunk):
        #     b_pts = pts_dynamic[b_idx: min(b_idx + chunk, pts_dynamic.shape[0]), ...]
        #     b_coords = my_voxel_tool.world2smoke(b_pts)
        #     cur_vel = vel_model(b_coords)
        #     delta_t = 1.0 / 150
        #     # b_bt_pts = b_pts - torch.cat((cur_vel, torch.full((cur_vel.size(0), 1), 1, device=cur_vel.device)),dim=-1) * delta_t
        #     # b_bt_coords = my_voxel_tool.world2smoke(b_bt_pts)
        #     b_bt_coords = b_coords - torch.cat((cur_vel, torch.full((cur_vel.size(0), 1), 1, device=cur_vel.device)),dim=-1) * delta_t
        #     mask = (b_bt_coords >= -1) & (b_bt_coords <= 1)
        #     mask = mask.all(dim=1)
        #     b_bt_coords[~mask] = b_coords[~mask]
        #     # b_bt_pts[~mask] = b_pts[~mask]
        #     b_bt_pts = my_voxel_tool.smoke2world(b_bt_coords)
        #     pts_dynamic[b_idx: min(b_idx + chunk, pts_dynamic.shape[0]), ...] = b_bt_pts
    ...
    # TODO: for resim, using F.grid_sample to get density
...
def render_rays_cuda_single_scene(ray_batch,
                ...
                chunk = 1024*64,
                perturb=0,
                my_voxel_tool = None, use_grid=False, den_grid=None
                ):
    ...
    if use_grid:
        ...
        # import pdb; pdb.set_trace()
        query_pts = torch.cat([xyzs_dynamic, rays_t_bc], dim = -1)
        coord = my_voxel_tool.world2smoke(query_pts)
        coord = coord[..., :3]
        den_grid = den_grid[None, ...].permute([0, 4, 3, 2, 1])  # [N, 1, Z, Y, X] i.e., [N, 1, D, H, W]
        den_sampled = F.grid_sample(den_grid, coord[None, ..., None, None, :], align_corners=True)
        smoke_density = den_sampled.reshape(C_smokeRaw[..., 3:].shape)
    else:
        smoke_density = C_smokeRaw[..., 3:] 
    # smoke_density = torch.relu(smoke_density)
...
def render_rays_single_scene(ray_batch,
    ...
    # mask = near < far
    # mask = mask.squeeze()
    # rgb_map[mask] = torch.tensor([1.0, 1.0, 1.0], dtype=rgb_map.dtype, device=rgb_map.device)
    # rgb_map[~mask] = torch.tensor([0.0, 0.0, 0.0], dtype=rgb_map.dtype, device=rgb_map.device)
...
def render_path(model, render_poses, hwf, K, chunk, near, far, cuda_ray, netchunk = 1024 * 64, gt_imgs=None, savedir=None, render_factor=0, render_steps=None, bkgd_color=None, vel_model = None, my_voxel_tool = None):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        t = time.time()
        rgb, disp, acc, extras = render(H, W, K, model, chunk=chunk, c2w=c2w[:3,:4], netchunk=netchunk, time_step=cur_timestep, bkgd_color=bkgd_color, near = near, far = far, cuda_ray = cuda_ray, perturb=0, vel_model = vel_model, my_voxel_tool = my_voxel_tool)
...
def render_2d_trajectory(model, render_poses, hwf, K, chunk, near, far, cuda_ray, netchunk = 1024 * 64, gt_imgs=None, savedir=None, render_factor=0, render_steps=None, bkgd_color=None):
    ...
    for idx, i in enumerate(tqdm(range(len(render_poses) -1, strat_frame, -1))):
        ...
        map_pts2d = project_points(mapped_points_3d_merged.detach(), torch.tensor(K, dtype=torch.float32).cuda(), c2w=c2w[:3,:4])
