...
def visualize_all(args, model, voxel_writer, t_info, global_step, targetdir=None):
    if targetdir is not None:
        args.full_vol_output = True
        print('visualize_all')
        testsavedir = os.path.join(targetdir, 'vis_summary_{:06d}'.format(global_step+1), 'vis_mapping')
        os.makedirs(testsavedir, exist_ok=True)
        visualize_mapping(args, model, testsavedir, voxel_writer, t_info)
        
        testsavedir = os.path.join(targetdir, 'vis_summary_{:06d}'.format(global_step+1), 'vis_feature')
        # os.makedirs(testsavedir, exist_ok=True)
        visualize_feature(args, model, testsavedir, voxel_writer, t_info)
        
        testsavedir = os.path.join(targetdir, 'vis_summary_{:06d}'.format(global_step+1), 'vis_velocity/')
        os.makedirs(testsavedir, exist_ok=True)
        output_voxel(args, model, testsavedir, voxel_writer, t_info, voxel_video = True)
        
        testsavedir = os.path.join(targetdir, 'vis_summary_{:06d}'.format(global_step+1), 'eval_mapping')
        os.makedirs(testsavedir, exist_ok=True)
    else:
        args.full_vol_output = True
        print('visualize_all')
...
def test(args):
    ...
    if args.mesh_only:
        ...
        testsavedir = os.path.join(targetdir, 'volumeout_{:06d}'.format(start+1))
        output_voxel(args, model, testsavedir, voxel_writer, t_info, voxel_video = args.voxel_video)
    elif args.visualize_feature:
        testsavedir = os.path.join(targetdir, 'vis_feature_{:06d}'.format(start+1))
        os.makedirs(testsavedir, exist_ok=True)
        visualize_feature(args, model, testsavedir, voxel_writer, t_info)
        
    elif args.visualize_mapping:
        testsavedir = os.path.join(targetdir, 'vis_mapping_{:06d}'.format(start+1))
        visualize_mapping(args, model, testsavedir, voxel_writer, t_info=t_info)
    elif args.render_only:
        testsavedir = os.path.join(targetdir, 'renderonly_{}_{:06d}'.format('test' if args.render_test else 'path', start+1))
        if args.render_eval:
            testsavedir = os.path.join(targetdir, 'renderonly_{}_{:06d}'.format('eval', start+1))
